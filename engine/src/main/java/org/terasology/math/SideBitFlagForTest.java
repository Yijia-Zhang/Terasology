 ...
 *
 */
public final class SideBitFlagForTest {
    ...
    public TObjectByteMap<Side> getSideBits(){
        return sideBits;
    }

    public byte getReverse(byte sides) {
        return (byte) ((sides / 8) + ((sides % 8) * 8));
    }

    public byte getSides(Set<Side> sides) {
        byte result = 0;
        for (Side side : sides) {
    ...
    public byte getSides(Side... sides) {
        byte result = 0;
        for (Side side : sides) {
    ...
    public boolean hasSide(byte sideBit, Side side) {
        return (sideBit & sideBits.get(side)) > 0;
    }

    public byte addSide(byte sideBit, Side... sides) {
        byte result = sideBit;
        for (Side side : sides) {
            result |= sideBits.get(side);
        }
